/* ==========================================================================
   SISTEMA MASTER PRO V21.0: MOBILE PREMIUM DRAWER & PERFECT POSITIONS
   Comunidade Aprender e Cuidar / Profissão Pet (Circle.so)
   ========================================================================== */

(function() {
    const oldStyles = document.querySelectorAll('style[id*="consolidated"], style[id*="legacy"], style[id*="pet-styles"], style[id*="pet-modal-styles"], style[id*="pet-modal-multi"], style[id*="sandbox"], style[id*="pet-anim"], style[id*="pet-widget-combined-styles"], style[id*="pet-master-system-styles"]');
    oldStyles.forEach(st => st.remove());

    if (document.getElementById("pet-master-system-styles-pro-v21")) return;
    var style = document.createElement('style');
    style.id = "pet-master-system-styles-pro-v21";
    style.innerHTML = `
        ...
        .pet-tour-card {
            ...
            animation: modalBounce 0.5s; word-wrap: break-word; overflow-wrap: break-word; position: relative;
        }
        ...
        .pet-tour-dandara-side {
            ...
            animation: dandaraStandFloat 4s ease-in-out infinite, modalBounce 0.6s;
        ...
        .pet-dandara-interactive-bubble {
            ...
            margin-bottom: 14px; border-left: 3px solid #e08b26; animation: speechPop 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275);
        ...
        .paw-stamp { display: inline-block; animation: stampIn 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards; }

        .pet-form-card { background: #ffffff; width: 92%; max-width: 500px; border-radius: 28px; box-shadow: 0 25px 60px -10px rgba(15, 23, 42, 0.45); overflow: hidden; display: flex; flex-direction: column; animation: modalBounce 0.6s cubic-bezier(0.175, 0.885, 0.32, 1.275); position: relative; }
        ...
        .pet-image-preview-img { width: 100%; height: 100%; object-fit: cover; object-position: center 25%; display: block; animation: modalBounce 0.4s; }
...
window.PetMasterSystem = {
    ...
    // POSES REAIS EM PNG TRANSPARENTE HOSPEDADAS NO GITHUB (PET-Rocinha-assets)
    poses: {
    ...
    tourLocations: [
        {
            ...
            selectors: ["#sidebar-primeiros-passos", "a[href*='primeiros']", "a[href*='start']"],
            pose: "boasVindas"
        },
        {
            title: "👤 Seu Perfil & Formulário Socioeconômico",
            desc: "No menu <b>Meu Perfil > Perfil Aluna</b> você preenche seu formulário socioeconômico para liberar seu auxílio financeiro de R$ 100,00, suas Medalhas e o Censo Pet!",
            breadcrumb: "📍 Menu Lateral > Meu Perfil > Perfil Aluna",
            selectors: ["#sidebar-perfil", "a[href*='perfil']", "a[href*='profile']"],
            pose: "apontando"
        },
        {
            ...
            selectors: ["#nav-cursos", "#nav-painel", "a[href*='cursos']", "a[href*='dash']"],
            pose: "apontando"
        },
        {
            title: "🏆 Compartilhe Vitórias & Tire Dúvidas",
            desc: "Comemore suas conquistas com a gente em <b>#Arrasei</b> e tire suas dúvidas com as mentoras no menu <b>Dúvidas</b>!",
            breadcrumb: "📍 Menu Lateral > Geral > #Arrasei / Dúvidas",
            selectors: ["#sidebar-arrasei", "#sidebar-duvidas", "a[href*='arrasei']", "a[href*='duvidas']"],
            pose: "comemorando"
        },
        {
            ...
            selectors: ["#pet-floating-widget", "#sidebar-perfil"],
            pose: "comemorando"
        }
    ],
    ...
    capturarEmailRobusto: function() {
        ...
        try {
            ...
            if (window.current_user?.email) { currentSystemEmail = String(window.current_user.email).toLowerCase().trim(); }
    ...
    findTargetElement: function(selectors) {
    ...
    renderTourStep: function(stepIndex) {
        ...
        const targetEl = this.findTargetElement(loc.selectors);
        ...
        if (targetEl) {
            ...
            if (rect.left > window.innerWidth / 2) {
                cardLeft = Math.max(20, rect.left - 620) + 'px';
                cardTop = Math.min(window.innerHeight - 360, Math.max(20, rect.top)) + 'px';
                transform = 'none';
            } else {
                cardLeft = Math.min(window.innerWidth - 620, rect.right + 20) + 'px';
                cardTop = Math.min(window.innerHeight - 360, Math.max(20, rect.top)) + 'px';
                transform = 'none';
            }
    ...
    exibirTravaSocioeconomicoPopup: function() {
        ...
        const modalHtml = `
            <div id="circle-popup-socoeco" class="pet-overlay-global" style="background: rgba(0,0,0,0.65); backdrop-filter: blur(5px);">
                <div role="dialog" aria-modal="true" aria-labelledby="popup-socoeco-title" style="background: white; width: 90%; max-width: 500px; border-radius: 24px; padding: 32px; text-align: center; box-shadow: 0 20px 50px rgba(0,0,0,0.4); animation: circleModalPop 0.3s cubic-bezier(0.16, 1, 0.3, 1);">
