...
var PetMasterSystem = {
    ...
    enableSandbox: function(customEmail) {
        ...
        console.log("🧪 PetMasterSystem: MODO SANDBOX ATIVADO! (Email Sandbox: " + this.sandboxEmail + ")");
        
        const key = this.getUserOnboardingKey(this.sandboxEmail);
        this.safeStorage('remove', key);
        this.safeStorage('remove', this.constants.LS_ONBOARDING_DONE);
        this.safeStorage('remove', this.constants.LS_PARTICIPADO);
        
        this.init();
        this.forceStartOnboarding();
        return "🧪 Modo Sandbox ativado com sucesso para: " + this.sandboxEmail;
    },

    disableSandbox: function() {
        this.sandboxMode = false;
        console.log("🧪 PetMasterSystem: Modo Sandbox Desativado.");
    ...
    ativarFalaDandara: function(parentWrap, duracaoMs) {
        ...
        try {
            ...
            this.talkingTimer = setTimeout(function() {
                if (parentWrap) parentWrap.classList.remove('pet-dandara-speaking');
            }, duracaoMs || 3500);
        } catch(e) {}
    },
    ...
    verificarEIniciarOnboarding: function() {
        ...
        if (urlParams.get('sandbox') === 'true') {
            this.sandboxMode = true;
            console.log("🧪 PetMasterSystem: Sandbox ativado via URL (?sandbox=true)");
        }
    ...
    renderizarWidget: function(data) {
        ...
        const sandboxHtml = this.sandboxMode ? `<span class="pet-sandbox-tag">🧪 SANDBOX</span>` : '';
